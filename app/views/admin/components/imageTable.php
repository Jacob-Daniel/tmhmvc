...
function buildImageTable(mysqli_result $images, bool $isGalleryView): string
{
    ob_start();
    ...
    while ($image = $images->fetch_object()) {
        ...
        <div class="relative group bg-white rounded overflow-hidden hover:shadow-lg transition-shadow duration-200">
            ...
            <div class="p-3 space-y-1">
                ...
                <?php if ($alt): ?>
                    <p class="text-xs text-gray-500 truncate" title="Alt: <?= $alt ?>">
                        <span class="font-semibold">Alt:</span> <?= $alt ?>
                    </p>
                <?php endif; ?>
            </div>

            <!-- Hidden field for batch operations -->
            <input type="hidden" name="image_gal_array[]" value="<?= $imagefile ?>">
        </div>
